...
for iwin = 1:number_of_windows
    ...
    % Calculate kurtosis of the signal values
       % feature_table.sampleKurtosis(iwin, 1) = kurtosis(current_signal);
        ...
        feature.signalEntropy(iwin, 1) = signal_entropy(current_signal');

        % Calculate spectral entropy of the signal
        feature.spectralEntropy(iwin, 2) = spectral_entropy(current_signal, fs, 256);

       
    % Extract features from the power spectrum
    [maxfreq, maxval, maxratio] = dominant_frequency_features(current_signal, fs, 256, 0);
      feature.dominantFrequencyValue(iwin, 3) = maxfreq;
        feature.dominantFrequencyMagnitude(iwin, 4) = maxval;
        feature.dominantFrequencyRatio(iwin, 5) = maxratio;
    ...
    features(iwin, 6) = MFCCs(1);
    features(iwin, 7) = MFCCs(2);
    features(iwin, 8) = MFCCs(3);
    features(iwin, 9) = MFCCs(4);
    features(iwin, 10) = MFCCs(5);
    features(iwin, 11) = MFCCs(6);
    features(iwin, 12) = MFCCs(7);
    features(iwin, 13) = MFCCs(8);
    features(iwin, 14) = MFCCs(9);
    features(iwin, 15) = MFCCs(10);
    features(iwin, 16) = MFCCs(11);
    features(iwin, 17) = MFCCs(12);
    features(iwin, 18) = MFCCs(13);
end